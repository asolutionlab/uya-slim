...
**当前实现**：`lib/std/async.uya` 单文件占位（`Future<T>`、`Waker`、**`Poll_i32`** 空/占位），满足 `@async_fn` 返回 `!Future<T>` 及类型检查；**`union Poll_i32 { Pending: void, Ready: i32 }`** 已提供，`Poll<T>` 待泛型 union 支持后实现。以下为**目标**目录结构，后续按阶段拆分实现。
