...
- `src/main.uya` 的目标态只负责命令发现、路径解析、参数转发、全局帮助和版本信息，不再解析 `build/run/test/fmt/upm` 的业务参数。
- 编译器业务逻辑由 `cmd/build` 与共享 compiler driver 模块独立承载；`cmd/run` / `cmd/test` 复用该驱动，不回调 `src/main.uya`。
- 过渡期保留历史隐式编译入口 `uya <file.uya> ...`，用于自举、`src/compile.sh` 和旧脚本兼容。
...
- 不要求第一阶段就删除 `src/main.uya` 的内部编译/隐式入口；为避免自举死锁，主程序可暂时保留隐藏兼容路径。
- 不新增公开 `compile` 子命令，除非后续单独立项；编译器内核优先作为 `cmd/build` 的共享驱动模块落地。
...
### 4.4 `src/main.uya` 的纯分发边界

设计结论：长期看，`src/main.uya` 应该只负责命令分发；编译器本体也应从公开入口中独立出去，由 `cmd/build` 或共享 compiler driver 处理。

- **过渡态**：`src/main.uya` 公开子命令全部外置，但保留隐藏/隐式编译入口，保证 `from-c`、`make uya`、`src/compile.sh` 能在 `bin/cmd/*` 尚未存在时继续构建。
- **目标态**：冷启动 seed 或备份流程可以同时生成 `bin/uya` 与 `bin/cmd/build` 后，`src/main.uya` 删除公开编译业务，只保留 dispatcher、帮助、版本和必要错误提示。
- **禁止回退**：`uya build/run/test/fmt/upm` 一旦进入外置化阶段，不应在 `cmd/xxx` 缺失时静默走内部实现；缺失时返回错误并提示 `make cmds` 或重新安装。
...
`src/main.uya` 的目标职责是 launcher/dispatcher，不拥有公开小命令业务逻辑。进入 `main()` 后只做最小分流：

1. `argc < 2`：打印主帮助。
2. `argv[1]` 是 `build/run/test/fmt/upm`：调用 `dispatch_cmd(argv[1], drop_first_subcommand=true)`，不解析这些命令的业务选项。
3. `argv[1]` 是 `--version` / `-v`：主程序直接处理。
4. `argv[1]` 是 `pack-image` / `inspect-image` / `verify-image` / `--outlibc`：第一阶段仍由主程序直接处理，后续可按同样模式外置。
5. 过渡期其他情况：保留隐藏的隐式编译入口，用于 `uya file.uya -o out`、`src/compile.sh` 和自举；目标态在 seed 能生成 `cmd/build` 后移除此公开依赖。
...
三者应共享编译驱动，避免复制 `src/main.uya` 的大量实现。`cmd/build` 是真正的编译器命令入口，不应再把参数转回 `src/main.uya`。推荐结构：
...
- `src/main.uya` 不再解析公开 `build/run/test/fmt/upm` 的业务参数；若仍保留隐式编译入口，应在代码和文档中标注为 bootstrap/compat 路径。
