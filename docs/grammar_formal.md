...
> **版本**：与 [uya.md](./uya.md) 0.49.5 同步（2026-03-19）
...
- 算术运算 `+`、`-`、`*`、`/` 可用于相同类型的 `@vector(T, N)`，结果类型保持不变；`%` 仅可用于**整数元素**的相同类型 `@vector(T, N)`，按通道取模，结果类型保持不变
- 位运算 `&`、`|`、`^`、`<<`、`>>` 与一元 `~` 仅适用于整数元素类型的 `@vector(T, N)`；一元 `-` 适用于整数或浮点元素类型的 `@vector(T, N)`
- 比较运算 `==`、`!=`、`<`、`<=`、`>`、`>=` 可用于相同类型的 `@vector(T, N)`，结果类型为 `@mask(N)`
- 掩码运算 `&`、`|`、`^`、`!` 可用于 `@mask(N)`
- `@vector.splat(x)` 通过上下文目标类型构造向量值；参数类型须与元素类型 `T` 一致或可隐式转换；无后缀浮点字面量为 `f64`，`f32` 向量须使用 `f32` 后缀（如 `1.0f32`）；目标类型还可由同一代数/比较/取模表达式中另一侧的 `@vector` 操作数推断
