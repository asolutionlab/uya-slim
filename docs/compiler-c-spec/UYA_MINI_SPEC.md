...
**类型极值（@max/@min）**：
- 整数类型（`i8`, `i16`, `i32`, `i64`, `u8`, `u16`, `u32`, `usize`, `u64`, `byte`）支持通过 `@max` 和 `@min` 内置函数访问极值
...
- **整数字面量**：默认类型为 `i32`，支持以下进制：
  - 十进制：`[0-9]+`，示例：`0`、`42`、`100`、`1_000_000`
  - 十六进制：`0x[0-9a-fA-F]+` 或 `0X[0-9a-fA-F]+`，示例：`0xFF`、`0x1A2B`、`0xDEAD_BEEF`
  - 八进制：`0o[0-7]+` 或 `0O[0-7]+`，示例：`0o755`、`0O644`、`0o7_777`
  - 二进制：`0b[01]+` 或 `0B[01]+`，示例：`0b1010`、`0B1111_0000`
  - **下划线分隔符**：数字中可以包含下划线 `_` 作为分隔符，提高可读性
    - 下划线可以出现在任意两个数字之间：`1_000_000`、`0xFF_00_AA`
    - 下划线不能出现在开头或结尾：`_123`、`123_` 是非法的
    - 下划线不能连续出现：`1__000` 是非法的
    - 下划线在解析时会被忽略，不影响数值的大小
  - 所有整数字面量的类型都是 `i32`（除非上下文需要其他整数类型）
  - 所有整数字面量在编译期转换为统一的内部表示（十进制值）
- **浮点字面量**：包含小数点或指数部分的数字，默认类型为 `f64`，示例：`3.14`、`1.0e-10`、`2.5E3`、`1_000.5`
  - 浮点字面量只支持十进制表示（不支持 `0x1.0p10` 等十六进制浮点）
  - 浮点字面量也支持下划线分隔符：`3.141_592_653`、`1.0e1_0`
...
运算符列表：
+  -  *  /  %        // 算术运算符
+| -| *|             // 饱和运算（溢出时返回类型极值），规范 uya.md §10、§16
+% -% *%             // 包装运算（溢出时模 2^n 包装），规范 uya.md §10、§16
<< >>               // 位移运算符（左移、右移）
== != < > <= >=     // 比较运算符
&  |  ^             // 位运算符（按位与、或、异或）
&& ||               // 逻辑运算符
...
- **饱和运算**（`+|`、`-|`、`*|`）：操作数必须为整数类型（i8/i16/i32/i64），两操作数类型一致；溢出时返回类型最大值（上溢）或最小值（下溢）。
- **包装运算**（`+%`、`-%`、`*%`）：操作数必须为整数类型（i8/i16/i32/i64），两操作数类型一致；溢出时按模 2^n 包装。
...
- **字段访问**：`expr.field_name`
  ...
  - **自动解引用**：如果左侧表达式是指针类型（`&StructName`），编译器会自动解引用（等价于 `(*expr).field_name`）
  - 字段名必须是结构体中定义的字段
  - 结果类型为字段的类型
...
- **运算符说明**：
  ...
  - 赋值：`=`（左侧必须是 `var` 变量或字段访问，类型必须匹配）
    - 支持字段赋值：`obj.field = value`（如果 `obj` 是结构体类型）或 `ptr.field = value`（如果 `ptr` 是指向结构体的指针类型）
...
- **函数调用**：
  ...
  - **可变参数函数调用**（uya.md §5.4）：
    ...
    - 可变参数部分可以传递任意数量的额外参数，或使用 `...` 转发：`printf(fmt, ...)`
...
1. **赋值类型匹配**：
   ...
   - **字段赋值**：
     - `obj.field = value;` ✅（如果 `obj` 是结构体类型）
     - `ptr.field = value;` ✅（如果 `ptr` 是指向结构体的指针类型，自动解引用）
     - 示例：`arena.buffer = buffer as &byte;`、`arena.size = size;`
...
10. **指针类型规则**：
   - `&T` 类型可用于普通变量和函数参数
   - `&void` 类型是通用指针类型，可以转换为任何指针类型（`&void` → `&T`），用于实现类型擦除和通用指针操作
   ...
   - 指针解引用和字段访问：
     - 使用 `*expr` 解引用指针（通用方式）
     - 使用 `ptr.field` 访问字段（自动解引用，等价于 `(*ptr).field`，当 ptr 是指向结构体的指针时）
     - 支持字段赋值：`ptr.field = value`（自动解引用后赋值）
     - 示例：`*ptr`、`ptr.x`（如果 ptr 是 `&Point` 类型，自动解引用）、`ptr.x = 10`（字段赋值）
...
| 结构体 | 支持（接口实现、方法块实现接口方法；drop 只能在结构体内部或方法块中定义） | 完整支持（方法、接口实现、drop） |
...
- **基于 Uya 规范版本**：0.42（内置函数以 @ 开头；数组类型与重复字面量统一为 `[T: N]`、`[value: N]`；参数列表即元组、可变参数与字符串插值见 uya.md；联合体 union 见 uya.md §4.5；drop 只能在结构体/联合体内部或方法块中定义，见 uya.md §12；方法 self 统一为 &T，*T 仅 FFI，见 uya.md §2、§6；泛型语法见 uya.md §B.1；宏系统见 uya.md §25；只读指针类型 `&const T` 和 `*const T` 见 uya.md §2.1.1、§5.2；函数导出规则见 uya.md §5.1）
- **目的**：Uya Mini 编译器自举实现
...
- **更新说明**：
  ...
  - 2026-01-16：类型检查器改进
    - 添加 `&void` 类型支持（通用指针类型，可转换为任何指针类型）
    - 改进指针类型转换规则（支持 `&void` ↔ `&T` 转换）
    - 改进字段访问类型检查（支持指针自动解引用）
    - 改进赋值语句类型检查（支持字段访问作为赋值目标）
  - 2026-02-01：error_id 稳定性规范（与 uya.md 同步）
    - `error_id = hash(error_name)`，相同错误名在任意编译中映射到相同值
    - hash 冲突时编译器报错并提示冲突名称
