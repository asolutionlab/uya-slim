...
- `expr`：要打印的表达式，支持以下类型：
  ...
  - 浮点类型：`f32`、`f64`
  - 布尔类型：`bool`
  - **C 风格字节串**（**0.49.41** 与实现对齐）：元素为 **`i8` / `u8` / `byte`** 的 **`&[T]`**、**`[T: N]`**；**`*byte`**（及指向 **`i8`** 的 FFI 指针，与现有规则一致）；字面量 **`"..."`**。其中 **`[byte: N]`**、**`&[byte]`** 变量以 **`%s`** 打印（需以 **`\\0`** 结尾的缓冲区行为与 C 一致）。
...
**本文档由 Uya 编译器团队维护，最后更新：2026-03-22（0.49.41）**
