...
- `expr`：要打印的表达式，支持以下类型：
  ...
  - 浮点类型：`f32`、`f64`
  - 布尔类型：`bool`
  - 字符串：`&[i8]`（字节切片）、`*i8`（C 字符串指针）、`[i8: N]`（字符数组）
...
- 第一阶段支持向量算术（含整数元素的按通道 `%`）、整数向量位运算、向量比较；一元 `-` 可用于整数/浮点元素向量，一元 `~` 仅用于整数元素向量
- 比较结果类型为 `@mask(N)`
...
- `@vector.splat(x)` 的目标向量类型必须能由上下文唯一确定（含与同一代数/比较表达式中另一侧 `@vector` 操作数对齐推断）
...
**本文档由 Uya 编译器团队维护，最后更新：2026-03-19（0.49.5）**
