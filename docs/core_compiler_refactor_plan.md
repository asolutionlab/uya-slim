...
- CLI 编排：`build` / `check` / `run` / `test` / `fmt` 等命令。
- 包管理：`cmd.upm.upm_lib`。
...
- 将 formatter 维持为独立工具，例如 `tools/fmt.uya` / `uyafmt`。
...
- `.tmp_fmt_input.uya` / `.tmp_fmt_writeback.uya`：临时 formatter smoke 文件；若已有正式测试覆盖，可删除或迁入 `tests/` 后改名。
