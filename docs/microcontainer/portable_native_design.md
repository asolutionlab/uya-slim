...
- `MICROAPP_TARGET_ARCH` 驱动的目标 gcc 编译链
- `.pobj -> .uapp` 打包链
- 镜像头、校验、加载模拟路径
- hosted loader 与 native payload 对照运行路径
...
3. 当前 `sim_exec_loaded()` 只做入口和 rodata 校验，没有真正跳转到容器入口执行。
4. 当前 hosted 路径里，真正执行的是对照 ELF，而不是 `.uapp` 中被映射后的 payload。
...
现有 `.uapp` 应升级为支持真实 native 执行的 `v2` 布局。
...
- 引入 `MicroAppTargetProfile`
- 把 `MICROAPP_TARGET_ARCH` 升级为 `profile` 选择的一部分
- 从当前“抽 `.text/.rodata`”升级为“提取 segment + reloc + entry_va”
- `run --app microapp` 改为真执行 `.uapp`
- 对照 ELF 路径改成显式调试选项，不再作为默认运行路径
...
- [ ] 冻结 `MicroAppTargetProfile` 结构
- [ ] 冻结 `.uapp v2` 头与段布局
- [ ] 冻结 bridge ABI 形式：`trap bridge` / `call-gate bridge`
...
- [ ] 为 `hard-vm` profile 切到 `PIC/PIE` payload 构建
- [ ] 从 ELF 提取 relocation 表
- [ ] 抽取 `.data/.bss`
- [ ] 将 `@syscall` lowering 改为正式 bridge ABI
- [ ] 新增 `std.microapp` 可移植 API 第一版

### 阶段 3：hosted x86_64 真执行

- [ ] `linux_x86_64_hardvm` profile 落地
- [ ] loader 使用 `mmap/mprotect` 映射容器地址空间
- [ ] 实现 x86_64 call gate
- [ ] `sim_exec_loaded()` 真正跳到 `entry_va`
- [ ] 回归用例：不再启动对照 ELF，也能打印和 `yield`
