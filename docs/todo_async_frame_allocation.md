...
- [ ] **在 lowering 阶段计算并记录每个 async 函数的 `frame_size` / `frame_align`；若新增 lowering 模块可放在 `src/lower/async.uya`，否则先落在现有 `src/codegen/c99/async_transform.uya` / `src/codegen/c99/function.uya`**
...
- [ ] 定义 `AsyncFrameMeta`
- [ ] **将 `AsyncFrameMeta` 与 `AsyncFrameFieldMeta` 定义放在 `src/checker/` 下，codegen 侧只做填充**
...
- [ ] **定义 `async_frame_pool_alloc(pool, frame_id)`，由运行时内部根据 `frame_id` 查静态 descriptor 表得到 `size/align`**
- [ ] 定义 `async_frame_pool_free(pool, ptr, frame_id)`
- [ ] 定义 `async_frame_pool_reset`

### 2.2 选择池策略

- [ ] **按 descriptor 的 `(size, align)` 做 size class 分桶，每桶一个固定容量 free list；align 不同的 frame 不能共享同一桶；`frame_id` 用于查 descriptor，不直接等同于 size class**
...
- [ ] 增加 `ASYNC_FRAME_POOL_CAP`（每桶最大帧数，采用 lazy commit 避免空转 RSS 占用）
...
- [ ] 为 async 函数生成 `*_poll`
- [ ] 让 `*_future_new` 只负责创建 first-class future（保留符号保证外部 C ABI 兼容，默认 pool，debug 开关下才允许 heap；绝不返回内部栈帧）
- [ ] 增加 `*_future_init_into(storage, args...)` 或等价内部入口，供 caller-owned storage 路径复用
- [ ] 让 `@frame(foo)` 可用于普通变量、结构体字段与池中的显式声明
- [ ] **生成 `*_frame_drop_fields` 函数，保证 parent frame 释放时先递归清理仍然 live 的 child future 字段**
- [ ] **生成统一 `future_release` / `frame_release` 包装，根据 ownership tag 归还 pool / debug heap；字段清理函数不释放当前 frame 本体**
...
- [ ] **对 `var f = foo(); @await f;` 等“变量暂存后再 await”场景，若变量未逃逸且生命周期在单一会话内，仍走栈分配**
...
- [ ] pool 满时 release 默认返回 `PoolFull` 或触发 scheduler 背压；debug heap fallback 只在显式调试开关下启用，并记录计数器
...
- [ ] 保证 child future 比 parent 先释放（通过 `*_frame_drop_fields` 递归保证）
- [ ] 为需要显式 drop 的 child future 字段生成 `*_live` 或等价标志，Ready/Error/cancel 清理后立刻清零，避免 double drop

### 4.3 失败处理

- [ ] release 默认 pool 满时返回 `PoolFull` 或交给 scheduler 背压
- [ ] debug 模式 pool 满时允许回退调试堆并累加计数器
- [ ] 绝不静默悬挂
- [ ] 绝不双重释放
...
- [ ] **对生成 C 做 `rg free` 检查，确认热路径没有 `free(frame)`**
- [ ] 确认热路径没有 `malloc(sizeof(struct uya_async_...))`
- [ ] 确认仍保留必要的 `pool_alloc` / `stack` 逻辑
- [ ] 确认 `@frame(foo)` 生成的类型对齐与 frame layout 一致
- [ ] **确认 `sizeof(struct uya_async_...)` 只出现在 `pool_alloc` 或栈声明中，不出现在 `malloc` 参数中**
- [ ] 确认 `*_future_new` 生成路径不会把 `Future.data` 指向内部局部变量
...
- [ ] **增加 `ab -n 1000000 -c 500 http://localhost:8876/`（不带 `-k`）基线对比**
- [ ] `wrk` / `ab` 下 RSS 维持平稳
