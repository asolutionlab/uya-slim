# Uya 语言规范 0.49.41（完整版 · 2026-03-22）
...
- 字符串字面量：
  - **普通字符串字面量**：`"..."`，类型为 `*byte`（FFI 专用类型）；用于初始化时语义见下。
    ...
    - 支持转义序列：`\n` 换行、`\r` 回车、`\t` 制表符、`\\` 反斜杠、`\"` 双引号、`\0` null 字符
    - 不支持 `\xHH` 或 `\uXXXX`（未来支持）
    - **字符串字面量的赋值与初始化**（以下目标类型均合法，字面量自动带 `\0` 结尾）：
      - ✅ 可赋值给 `[byte: N]`：当「字面量可见字符数 + 1（\0）」≤ N 时，按字节拷贝到数组并保证以 `\0` 结尾；例如 `var buf: [byte: 8] = "hi";`
