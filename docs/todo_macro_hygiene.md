...
### 1.2 收集 local_binding 名

- [ ] 实现 `collect_local_binding_names(body: &ASTNode, arena: &Arena)`（或等价签名）：遍历宏体顶层，对 `const x = @mc_eval(...)` 和 `const x = @mc_type(...)` 收集 `x`，返回名字集合（可用固定大小数组或 arena 分配的表）。
...
- [ ] 实现 `build_hygiene_data(body, param_names, local_binding_names, arena, expansion_id)`（或通过 ctx 传入 param 名）：
  - [ ] 第一遍：遍历宏体 AST，收集所有绑定点（var_decl / for_stmt / fn_decl / fn_decl_params）；若名不在 param_names 且不在 local_binding_names，则分配 binding_id，写入 binding_sites，生成 `name__hyg_<expansion_id>_<binding_id>` 写入 rename_map。
  - [ ] 第二遍：作用域解析，对整棵 AST 中每个 `AST_IDENTIFIER` 解析到 binding_id / 参数 / 自由名，写入 use_sites。
- [ ] 作用域规则：BLOCK、FN_DECL 压栈；FN_DECL 先处理 params 再处理 body；for 循环变量按设计文档 §5 处理。
...
- [ ] **AST_CALL_EXPR 路径**（expand_macros_in_node_simple 内，约 2755–2778 行）：在构建好 `ctx.bindings` 后，先得到 `local_binding_names`（调用 collect_local_binding_names），再调用 `build_hygiene_data(...)`，将结果填入 `ctx` 的 hygiene 字段，再调用 `extract_macro_output_with_params`。
- [ ] **AST_METHOD_BLOCK 路径**（约 2862–2912 行）：对 struct 返回宏，在构建好 `ctx.bindings` 后同样调用 `collect_local_binding_names` 与 `build_hygiene_data(...)`，将结果填入 `ctx`，再调用 `extract_macro_output_with_params`。
...
- [ ] **AST_FN_DECL**：若启用卫生且在 binding_sites 中查到该 fn 节点，则 copy.fn_decl_name = rename_map[binding_id]；否则保持原名。fn_decl_params 的拷贝在递归时走 AST_VAR_DECL 分支，已由上面处理。

### 2.2 deep_copy_ast_with_field_subst

- [ ] 在拷贝 AST_IDENTIFIER、AST_VAR_DECL、AST_FOR_STMT、AST_FN_DECL 时，应用与 `deep_copy_ast_with_params` 相同的“是否启用卫生 + use_sites/binding_sites 查找 + rename_map”逻辑，保证 for info.fields 展开后的块内标识符也卫生。
...
- [ ] 新增 `tests/test_macro_hygiene.uya`：
  ...
  - [ ] 用例 3：同一宏在同一文件内展开两次，两次展开中宏内局部变量互不影响（例如两次各声明并使用的局部变量不互相覆盖）；通过返回值或可观测行为验证（如两次调用各自返回预期值、无互相干扰）。
...
- [ ] 在 [docs/uya.md](uya.md) 宏系统章节（§25）中增加“卫生宏”小节：说明宏体内引入的变量/循环变量/函数名/参数会做卫生重命名，避免与调用处名字捕获；宏参数与 `const x = @mc_eval/@mc_type` 的语义不变；生成名格式可注明（如 `__hyg_<expansion>_<id>`）。

### 4.2 语法规范文档

- [ ] 在 [docs/grammar_formal.md](grammar_formal.md) 中补充卫生宏相关说明：宏展开阶段对宏体内绑定的标识符进行卫生重命名的语义规则（可与 uya.md §25 对应）；若有宏相关 BNF 或语义小节，增加“卫生重命名”的简要描述。
...
- [ ] test_macro_hygiene.uya 中三则用例通过。
- [ ] uya.md §25 包含卫生宏说明。
