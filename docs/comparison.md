...
| `*byte`         | char*         | 4/8 B（平台相关） | FFI 指针类型 `*T` 的一个实例（T=byte），用于 FFI 函数参数和返回值，指向 C 字符串；字符串字面量可赋给 `[byte:N]`、`&byte`、`*byte`（自动 `\0` 结尾，见 uya.md §1.4）；可与 `null` 比较；FFI 指针类型 `*T` 支持所有 C 兼容类型（见 uya.md 第 5.2 章）|
