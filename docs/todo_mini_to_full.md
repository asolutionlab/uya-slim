...
- [~] **标准库实现**（基于核心类型，详见 [`docs/std_async_design.md`](./std_async_design.md)）
  ...
  - [~] `std.thread` 模块：最小版 `ThreadPool` + `async_compute_i32` 已落地（当前为懒启动的可复用 worker 进程池；worker 通过 `fork + pipe` 回传结果；共享状态中已包含固定任务槽位与共享 FIFO 队列，worker 侧通过“slot 索引”取任务并写回结果；当 worker 全忙时任务会先进入共享队列，父进程仍可按 FIFO 顺序为队首任务分发空闲 worker，而 worker 完成当前 slot 后也会继续尝试从共享队列续取下一个 slot；只有队列满时才回退 one-shot 子进程；已补 `thread_pool_shutdown` / `thread_pool_started_workers` / `thread_pool_pending_count`；`poll` 可见 `Pending -> Ready`，并已验证 `Scheduler + LinuxEpoll + TaskQueue_i32` 闭环，见 `test_std_thread.uya`；本轮还暴露了两个编译器侧缺口：其一，安全证明对循环中的共享槽位索引仍需要显式的局部边界证明，否则会报数组索引安全证明失败；其二，codegen 对 `self.pool._shared_state` / `self.pool._workers` 这类链式指针访问仍可能生成错误的 `.`/`->`，当前需先收成 `pool_ref` 局部引用规避）；通用 `async_compute<T>`、worker 自主消费的完整共享队列与 Send/Sync/跨线程验证仍待实现
