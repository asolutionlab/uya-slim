...
- [~] **标准库实现**（基于核心类型，详见 [`docs/std_async_design.md`](./docs/std_async_design.md)）
  ...
  - [~] `std.async.io` 模块：`AsyncWriter`, `AsyncReader` 接口 + `MemAsyncWriter` 实现（当前在 async.uya 中；非阻塞 AsyncFd 待实现）
...
- [ ] **测试用例**：
  ...
  - [x] `test_async_io.uya` - AsyncWriter/AsyncReader 接口与 MemAsyncWriter（write/flush 返回 Ready），通过 `--c99` 与 `--uya --c99`
