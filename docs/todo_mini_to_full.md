...
- **阶段 4**：真实 SIMD lowering（🔄 已启动）
  - ✅ **初版**：C99 在 **x86_64 + SSE2**（`UYA_HAVE_SIMD_X86_SSE`）或 **ARM + NEON**（`UYA_HAVE_SIMD_ARM_NEON`，同名 `uya_simd_sse_*`）或 **`#else` 标量**下，对 **4× / 8× / 16× / 32× / 64×**`i32`/`u32`/`f32`（8=2×x4 … 64=16×x4；**2×** 仍标量）部分运算、`splat`、一元 `-`、**六种向量比较**；见 `emit_simd_x86_sse_runtime_helpers`、`c99_simd_try_emit_x86_sse_*`、`c99_simd_sse_x4_tile_lane_count_ok`；验证 `verify_simd_c99_neon.sh`
  - 待办：**2×** x4 快路径、其他元素宽度、`load/store/select`、`shuffle/reduce`、ABI（**NEON**：✅ 0.49.16；**多宽 x4 分块**：✅ 至 64，0.49.17–0.49.19）
