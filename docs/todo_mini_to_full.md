...
- **阶段 4**：真实 SIMD lowering（🔄 已启动）
  ...
  - 待办：**`shuffle/reduce`**（**`@vector.load` / `@vector.store` / `@vector.select`**：✅ **0.49.33** / **0.49.34** / **0.49.35**；其余见 `uya.md`）；ABI（**NEON**：✅ 0.49.16；**多宽 x4 分块**：✅ 至 64；**`2×` i32/u32/f32**：✅ 0.49.29；**i8/u8/i64/u64 快路径**：✅ 0.49.30；其余同上 ✅）
