...
- **阶段 2**：编译器最小落地（✅ 第一阶段语义）
  ...
  - 测试：`test_simd_value_ops.uya`、`test_simd_unary_ops.uya`、`test_simd_div_f32_i32.uya`、`test_simd_fn_vector_return.uya`、`test_simd_struct_field_ops.uya`、`test_simd_splat_f32_suffix.uya`、`test_simd_splat_peer_infer.uya` 与 `error_simd_*.uya`（含 `error_simd_vector_mod.uya`）
- **阶段 3**：标准库性能试点
