...
- [~] **标准库实现**（基于核心类型，详见 [`docs/std_async_design.md`](./std_async_design.md)）
  ...
  - [x] `test_async_compute_types.uya` - 验证 `async_compute<T>` 新增类型（`i64`/`u64`/`i16`/`u16`/`i8`/`u8`/`bool`）通过 `scheduler_run_*_with_event_loop` 正确运行；`f32`/`f64` 因 usize 位宽转换问题暂时跳过
  - [x] `std.thread` 模块：`ThreadPool` 最小实现稳定；`async_compute_i32()` / `async_compute_u32()` / `async_compute_usize()` 与通用 `async_compute<T>() -> Future<!T>` 已落地；`T` 分发覆盖 `i32` / `u32` / `usize` / `i64` / `u64` / `i16` / `u16` / `i8` / `u8` / `bool`（`f32`/`f64` 待修复 usize 位宽转换）；`test_std_thread.uya` 与 `test_async_compute_types.uya` 在 `--c99` 与 `--uya --c99` 通过；`scheduler_run_*_with_event_loop` 特化覆盖所有新增类型；修复 codegen bug：`finish_from_raw_poll` 中 `thread_ok_*` 替代 `ok<T>` 内置调用（`ok<T>` 内置对 i64/u64/i16/u16/i8/u8 等类型错误生成 `std_ok_*` 符号）。待：评估 typed API 是否统一为接口返回、Send/Sync/跨线程验证。
