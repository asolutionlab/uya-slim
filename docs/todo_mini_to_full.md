...
- [~] **标准库实现**（基于核心类型，详见 [`docs/std_async_design.md`](./std_async_design.md)）
  ...
  - [~] `std.thread` 模块：最小版 `ThreadPool` + `async_compute_i32` 已落地（懒启动可复用 worker 进程池；`fork + pipe` 回传；共享状态含固定任务槽位、共享 FIFO 与 `worker_idx` 绑定；共享槽位任务的 worker 启动入口现已统一为“先入队，再由父进程唤醒空闲 worker 发送 drain 命令，worker 自己从共享队列取活”，worker 完成当前 slot 后也会继续从共享队列续取下一 slot；父进程会根据共享状态中的 `RUNNING + worker_idx` 回刷本地 `busy/active_slot`，避免 drain 中 worker 被误判为空闲；future 侧的 shared-slot 启动已进一步下沉到池级 helper，当前主要经 `thread_pool_submit_slot_i32()` / `thread_pool_try_progress_slot()` / `thread_pool_try_kick_drain()` 完成“提交 + 请求推进”，并在 future 内通过 `start_execution()` / `progress_current_slot()` / `finish_with_pending_wake()` / `finish_from_read()` 收口启动、slot 推进、已绑定 fd 的结果读取与完成后续唤醒，`poll()` 现在基本收敛成“未启动则 start、已提交未绑定则 bind/progress、已绑定则统一读结果”三段；同时 slot 进入 `DONE` 后 future 仍可经 `try_bind_slot_worker()` 迟绑定结果 fd，late-poll 回归也已刻意拉长时序窗口验证这一点；队列满回退 one-shot；`thread_pool_shutdown` / `thread_pool_started_workers` / `thread_pool_pending_count`；`poll`：`Pending -> Ready`；`Scheduler + LinuxEpoll + TaskQueue_i32` 见 `test_std_thread.uya`）。**安全证明**：worker 侧共享槽位循环、worker 状态回刷以及池级 / future 内 helper 中的 slot/worker 访问仍常需显式 `slot_idx` / `worker_idx` 边界。**C99**：多级成员访问 `.`/`->` 已修复（`src/codegen/c99/expr.uya`、`types.uya`；`test_c99_pointer_access.uya`）。待：通用 `async_compute<T>`、Send/Sync/跨线程验证等
