...
- **阶段 4**：真实 SIMD lowering（🔄 已启动）
  - ✅ **初版**：C99 在 **x86_64 + SSE2**（`UYA_HAVE_SIMD_X86_SSE`）或 **ARM + NEON**（`UYA_HAVE_SIMD_ARM_NEON`，同名 `uya_simd_sse_*`）或 **`#else` 标量**下，对 **4× / 8× / 16×**`i32`/`u32`/`f32`（8=2×x4，16=4×x4）部分运算、`splat`、一元 `-`、**六种向量比较**；见 `emit_simd_x86_sse_runtime_helpers`、`c99_simd_try_emit_x86_sse_*`；验证 `verify_simd_c99_neon.sh`
  - 待办：非 4/8/16 宽度、`load/store/select`、`shuffle/reduce`、ABI（**NEON**：✅ 0.49.16；**8×/16×**：✅ 2/4×x4，0.49.17 / 0.49.18）
