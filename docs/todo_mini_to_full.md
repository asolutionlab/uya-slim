...
- [~] **标准库实现**（基于核心类型，详见 [`docs/std_async_design.md`](./std_async_design.md)）
  ...
  - [~] `std.thread` 模块：最小版 `ThreadPool` + `async_compute_i32/async_compute_usize` 已落地（懒启动可复用 worker 进程池；`fork + pipe` 回传；共享状态含固定任务槽位、共享 FIFO 与 `worker_idx` 绑定；共享槽位参数/结果现统一按机器字宽 raw bits 传输，worker 依据 `task_kind` 分派 `i32(i32)` / `usize(usize)` 调用；共享槽位任务的 worker 启动入口现已统一为“先入队，再由父进程唤醒空闲 worker 发送 drain 命令，worker 自己从共享队列取活”，worker 完成当前 slot 后也会继续从共享队列续取下一 slot；父进程会根据共享状态中的 `RUNNING|DONE + worker_idx` 回刷本地 `busy/active_slot`，避免 drain 中 worker 或尚未被 future 消费结果 fd 的 DONE worker 被误判为空闲；future 侧的 shared-slot 启动已进一步下沉到池级 helper，当前主要经 `thread_pool_submit_slot_i32()` / `thread_pool_submit_slot_usize()` / `thread_pool_try_progress_slot()` / `thread_pool_try_kick_drain()` 完成“提交 + 请求推进”，并在 future 内通过 `start_execution()` / `progress_current_slot()` / `finish_with_pending_wake()` / `finish_from_read()` 收口启动、slot 推进、已绑定 fd 的结果读取与完成后续唤醒，`poll()` 基本收敛成“未启动则 start、已提交未绑定则 bind/progress、已绑定则统一读结果”三段；同时 slot 进入 `DONE` 后 future 仍可经 `try_bind_slot_worker()` 迟绑定结果 fd，late-poll 回归也已覆盖。当前导出的是 `AsyncComputeI32Future` / `AsyncComputeUsizeFuture` 具体 future 结构体（都实现 `Future<!T>`），这样可绕开当前 C99 路径里“函数直接返回 `Future` 接口对象”不稳的问题；队列满仍回退 one-shot；`thread_pool_shutdown` / `thread_pool_started_workers` / `thread_pool_pending_count`；`Scheduler + LinuxEpoll` 集成见 `test_std_thread.uya`）。**安全证明**：worker 侧共享槽位循环、worker 状态回刷以及池级 / future 内 helper 中的 slot/worker 访问仍常需显式 `slot_idx` / `worker_idx` 边界。**C99**：多级成员访问 `.`/`->` 已修复（`src/codegen/c99/expr.uya`、`types.uya`；`test_c99_pointer_access.uya`），当前 `std.thread` 继续采用导出具体 future struct 的方式规避接口返回临时值问题。待：通用 `async_compute<T>`、统一 one-shot/shared-slot 返回形态、Send/Sync/跨线程验证等
