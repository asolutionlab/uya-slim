...
- [~] **标准库实现**（基于核心类型，详见 [`docs/std_async_design.md`](./std_async_design.md)）
  ...
  - [~] `std.thread` 模块：最小版 `ThreadPool` + `async_compute_i32` 已落地（当前为 `fork + pipe` 的最小异步轮询路径，`poll` 可见 `Pending -> Ready`，并已验证 `Scheduler + LinuxEpoll + TaskQueue_i32` 闭环，见 `test_std_thread.uya`）；通用 `async_compute<T>` 与真正队列化线程池待实现
