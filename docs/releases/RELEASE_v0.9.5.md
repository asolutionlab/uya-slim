...
| `macOS arm64 CI` / `make microapp-hosted-smoke` | 已接线，当前被 bootstrap 阶段阻塞 | 公开 `macos-ci` 最近运行仍在旧的 `make from-c` 入口提前失败；工作树已修正为 `make from-c-native` |
| `macOS arm64 CI` / `make microapp-aarch64-runtime-check` | 已接线，待修正 bootstrap 后拿最终绿灯记录 | workflow 已接入 standalone 入口，但最近公开运行未真正执行到该步骤 |
| `macOS arm64 CI` / `make microapp-macos-runtime-check` | 已接线，待修正 bootstrap 后拿最终绿灯记录 | workflow 已接入 standalone 入口，但最近公开运行未真正执行到该步骤 |
