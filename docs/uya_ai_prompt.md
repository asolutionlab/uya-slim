...
- 调试：`@print`、`@println`
- 内联汇编：`@asm { ... }`、`@asm_target()`
- SIMD：`@vector(T, N)`、`@mask(N)`，以及 `@vector.splat(x)`、`@vector.load(ptr)`、`@vector.store(ptr, v)`、`@vector.any(m)`、`@vector.all(m)` 等（详见规范 §16 / `docs/builtin_functions.md`）
...
- **对应规范版本**：与 `docs/uya.md` 头部一致（当前 **0.49.34**）。
- **更新日期**：2026-03-19
