...
| `&[T]` | 8/16 B | 元素可变切片；**字符串字面量不可赋给 `&[byte]`**（须 **`&[const byte]`**，**0.49.43**） |
| `&[const T]` | 8/16 B | 元素只读切片，语法 **`&[` `const` `T` `]`**；字面量重绑、字符串赋切片用此 |
| `&[T: N]`、`&[const T: N]` | 8/16 B | 编译期已知长度上界的切片 |
| `&[T]`（表达式） | — | **0.49.41**：**`&"text"[start:len]`** 类型为 **`&[byte]`**（非 const）；逻辑基底长度=可见字节数+1（含 **`\\0`**），用于子区间视图 |
...
**字符字面量**：`'a'`, `'x'`, `'\n'`, `'\t'`, `'\r'`、**`\\xHH`**、**`\\uXXXX`**（**`\\u`** 仅当值 **≤255**，**0.49.42**）等（0.43 新增）
...
// 字符串字面量后缀下标/切片（0.49.41）：逻辑长度为「可见字节数+1」（含末尾 \0）
const sub: &[byte] = &"hello"[0:3];       // 表达式类型 &[byte]；与等长 [byte:N] 上 &buf[0:3] 一致
// 变量接字面量赋值须 &[const byte]（0.49.43），不可声明为 &[byte] 再 s = "x"
var ro: &[const byte] = "hi";
ro = "yo";                                 // 重绑 .ptr/.len，非 memcpy 原缓冲
```
...
extern printf(fmt: *byte, ...) i32;
printf("Hello\n");  // ✅ FFI 参数
// 支持转义：\n \t \r \\ \" \0、\xHH（两位十六进制字节）、\uXXXX（BMP→UTF-8；代理区非法）

// 原始字符串字面量（无转义）
printf(`C:\Users\name`);
```

**字符串字面量 → `[byte: N]`（初始化与 `=` 赋值，普通/原始字面量同规则）**（**0.49.43**）：
- **容量**：**`N ≥ 可见字节数 + 1`**（`+1` 为结尾 **`\\0`**）。原始字符串无转义，**逐字节**计数；普通字符串按展开后的字节数计（如 **`\\n`** 为 1 字节）。
- **左值**：可为标识符或**任意深度成员链**（如 **`obj.f`**、**`a.b.c`**），规则相同。
- **赋值语义**：整数组先清零再拷贝字面量（含 **`\\0`**），与初始化一致。

**字符串字面量 → 切片变量**（**0.49.43**）：
- **仅** **`var s: &[const byte] = "hello";`** 或 **`s = "hello";`**（类型为 **`&[const byte]`** 或 **`&[const byte: N]`**）。**禁止** **`&[byte]`** 接收字面量 **`=`**。
- **语义**：更新切片的 **`.ptr` / `.len`**，指向静态字面量存储；**不对**原缓冲区做整块 `memcpy`。

**只读切片元素**（**0.49.43**）：
- 若 **`s`** 的类型为 **`&[const T]`**，**禁止** **`s[i] = …`**（含 **`+=`** 等降级的赋值）。

**子区间视图（与上列独立）**（**0.49.41**）：
- 表达式 **`&"text"[start:len]`** → **`&[byte]`**；边界按「可见字节数+1」逻辑长度检查。不要用此条推断「变量可声明为 `&[byte]` 再接字面量 `=`」。

**其它**：
- ✅ **`&byte`**、**`*byte`**、FFI 参数、与 **`null`** 比较等仍按 **`docs/uya.md`**。
...
- **对应规范版本**：与 `docs/uya.md` 头部一致（当前 **0.49.43**）。
- **更新日期**：2026-03-26（本节同步字符串/`&[const byte]`/字面量赋值规则）
...
| 自举 libc `sprintf`/`vsprintf` | `lib/libc/stdio.uya`（**`vsprintf`** 不得对 **`_vfprintf_impl`** 传 **`buf_size==0`**；使用 **`_VSPRINTF_BUF_CAP`**） |
