...
实现**卫生宏**：宏体内**引入的**标识符（变量、循环变量、函数名、参数）在展开时被重命名为**唯一生成名**，使得：
...
| 绑定点 (binding site) | 在宏体内引入新名字的 AST 节点：`AST_VAR_DECL`、`AST_FOR_STMT`（循环变量）、`AST_FN_DECL`（函数名）、以及 `fn_decl_params[i]`（参数，即 `AST_VAR_DECL`）。 |
| 使用点 (use site) | 引用某个名字的 `AST_IDENTIFIER` 节点（包括作为 `member_access_object`、`mc_interp_operand` 等子节点出现时）。 |
| 宏局部 (macro-local) | 在宏体内绑定、且**不是**宏参数、也**不是**会进入 `local_bindings` 的 const（如 `const x = @mc_eval(...)`）的名字。 |
...
- `AST_VAR_DECL` 的 `var_decl_name`（**排除**会进入 `local_bindings` 的 `const x = @mc_eval(...)` / `const x = @mc_type(...)`）。
- `AST_FOR_STMT` 的 `for_stmt_var_name`。
- `AST_FN_DECL` 的 `fn_decl_name` 及 `fn_decl_params[i].var_decl_name`（参数节点类型为 `AST_VAR_DECL`）。
...
- **仅**处理 var / for / fn（及 fn 参数）。**不**处理 `struct_decl_name`、`enum_decl_name`、`interface_decl_name` 等类型声明名。
...
### 4.2 收集 local_binding 名

与 `extract_macro_output_with_params` 开头的逻辑一致（可抽成共用函数 `collect_local_binding_names(body, arena)`）：

- 遍历宏体**顶层**语句（body 为 `AST_BLOCK` 时的 `block_stmts[i]`）。
- 若为 `AST_VAR_DECL` 且 `var_decl_is_const != 0`、`var_decl_init` 为 `AST_MC_EVAL` 或 `AST_MC_TYPE`，则将该 `var_decl_name` 加入集合 L。
- L 用于在“绑定收集”时排除：名字在 L 中的绑定点**不**分配 binding_id，也不进入 binding_sites / rename_map（它们在 merged_ctx 中作为“形参”被替换）。

### 4.3 绑定收集（第一遍遍历）

- 递归遍历**整棵**宏体 AST，按遍历顺序为每个需重命名的绑定点分配 binding_id（即 next_id++），顺序无语义要求。
- 对每个**绑定点**：
  - `AST_VAR_DECL`：若 `var_decl_name` 不在宏参数名集合且不在 L，则分配 `binding_id = next_id++`，写入 `binding_sites`，并生成 `new_name` 写入 `rename_map[binding_id]`。
  - `AST_FOR_STMT`：对 `for_stmt_var_name` 同上（不在参数名、不在 L 则分配 binding_id 并重命名）。
  - `AST_FN_DECL`：对 `fn_decl_name` 同上；对 `fn_decl_params[i]`（每个为 `AST_VAR_DECL`）按参数名/L 判断，若为宏局部则分配 binding_id。
...
- **进入作用域**：`AST_BLOCK`、`AST_FN_DECL`（先处理参数再处理函数体，见下）。
- **退出作用域**：离开上述节点时弹栈。
- **AST_FN_DECL** 顺序：先将 `fn_decl_params` 中每个参数的 `var_decl_name` 与对应 binding_id（若为宏局部）或 -1（若为宏参数）加入当前作用域，再递归 `fn_decl_body`，这样函数体内引用的是参数或本层绑定。
...
- **AST_FN_DECL**：对 `fn_decl_name` 在 binding_sites 中查当前 fn 节点；对 `fn_decl_params[i]` 在递归拷贝时作为 `AST_VAR_DECL` 已由上面规则处理。
...
- **函数**：`AST_FN_DECL` 的参数先于函数体加入作用域；然后函数体作为新作用域。
- **for**：`for_stmt_var_name` 在“当前作用域”中视为一个绑定（与 Uya 语义一致：循环变量在该 for 所在作用域内）。
- 首版可仅对 **BLOCK** 与 **FN_DECL** 做显式“压栈/弹栈”；for 变量视为与所在块同一层或紧接在块内下一层，依实现而定，只要与 [grammar_formal.md](grammar_formal.md) / 类型检查器中的解析顺序一致即可。
...
1. **AST_CALL_EXPR**（约 2755–2778 行）：识别到宏调用、构建好 `ctx.bindings` 后，先调用 `collect_local_binding_names(body, arena)`（或内联等价逻辑），再调用 `build_hygiene_data(body, param_names, local_binding_names, arena, expansion_id)`，将返回的 use_sites、binding_sites、rename_map、count 填入 `ctx`，再调用 `extract_macro_output_with_params(body, &ctx, ...)`。
2. **AST_METHOD_BLOCK**（约 2862–2912 行）：对 struct 返回宏展开时，同样在构建好 `ctx.bindings` 后先得到 `local_binding_names`，再调用 `build_hygiene_data(...)` 并填入 `ctx`，再调用 `extract_macro_output_with_params`。
...
该函数用于 `for info.fields` 的展开，也会拷贝宏体片段。传入的 `ctx` 已包含同一次展开的 hygiene 数据。在拷贝 `AST_IDENTIFIER`、`AST_VAR_DECL`、`AST_FOR_STMT`、`AST_FN_DECL` 时，应用与 `deep_copy_ast_with_params` **相同**的查找与重命名规则（先判断 ctx 是否带 hygiene 数据，再查 use_sites / binding_sites）。
...
- **同名多绑定（遮蔽）**：不同作用域中同名变量对应不同 binding_id，rename_map 中各有唯一新名；作用域解析保证每个使用点只解析到一个 binding_id。
...
| 风险/点 | 说明 |
|---------|------|
| 作用域实现细节 | 若 if/else、match、catch 等引入新作用域，需与语法规范一致；首版可仅 BLOCK/FN_DECL 压栈。 |
| 性能 | use_sites / binding_sites 线性查找；宏体通常较小，若后续宏体变大可考虑按节点指针或 (line,col) 的简单哈希。 |
| 唯一名冲突 | 前缀 `__hyg` 与用户标识符冲突概率极低；若规范保留该前缀则可文档约定。 |
...
- 新增 `test_macro_hygiene.uya`：宏内局部变量与调用处同名不互相干扰；同一宏在同一文件内展开两次得到不同唯一名；宏参数与自由名行为与现有一致。
