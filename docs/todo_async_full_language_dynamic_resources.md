...
- [ ] `@async_fn` 体内支持完整 Uya 函数体语法，而不是只支持若干 lowering 特判组合。
  - [ ] 根据矩阵补齐剩余 async 函数体语法/语义缺口，并收口历史“已完成”口径。
    - [ ] 收口 async 函数体语法矩阵和历史“已完成”口径。
      - [ ] 按矩阵为第一个“同步合法但 async 未验证/缺失”的函数体语法补最小回归。
        - 验证：`../uya/bin/uya --c99 tests/<新增或更新用例>.uya`
        - 完成条件：新增或更新的回归能稳定暴露该语法缺口或证明已有实现覆盖。
      - [ ] 修复该语法缺口并同步矩阵。
        - 验证：`../uya/bin/uya --c99 tests/<新增或更新用例>.uya`；`make tests-uya`
        - 完成条件：对应矩阵项从“未验证/缺失”移动到“已覆盖”，且除规范明确禁止项外不新增 async 独有限制。
