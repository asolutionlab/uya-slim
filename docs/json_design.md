...
- **手写辅助**：`json_object_find_index(o, key_ptr, key_len) !usize`、`json_expect_i64(v) !i64`、`json_expect_str(v) !JsonStrView`（类型不符返回 WrongType）、`json_str_view_eq`；调用方在同一作用域内用 `o.pairs[idx].value`，不返回/存储 `&JsonValue`（零 UB）。
