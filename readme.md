...
- **泛型语法**：使用尖括号 `<T>`，约束紧邻参数 `<T: Ord>`，多约束连接 `<T: Ord + Clone + Default>`（`Vec<i32>`、`HashMap<K, V>`、`Result<T, E>`）
...
- **自举编译器**：已完成自举，所有测试通过。编译器能编译自身，输出 C99 代码。
...
- **最新特性**：越界访问检测、指令融合优化、冗余指令消除、RISC-V 平台扩展支持、编译期优化框架（v0.7.3-v0.7.4）。
...
**注意**：语言规范为完整版（0.72）；**v0.7.2** 发布的自举编译器已完全自举。完整特性与未来计划见 [docs/uya.md](./docs/uya.md)。
