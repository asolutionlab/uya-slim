...
if ! grep -q 'content_hash = "' "$LOCK_FILE"; then
    cat "$LOCK_FILE"
    exit 1
fi

test -d "$TMP_HOME/.uya/pkg/vcs"
find "$TMP_HOME/.uya/pkg/vcs" -mindepth 1 -maxdepth 1 | grep -q .

echo "verify_upm_global_cache_git: ok"
