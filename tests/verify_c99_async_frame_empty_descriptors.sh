...
if ! grep -q "static struct AsyncFrameDescriptor _uya_async_frame_descriptor_entries\\[1\\]" "$OUT_C"; then
    echo "empty async frame descriptor backing table should use only one placeholder entry"
    exit 1
fi
