...
- 在证明过程中跟踪步数和时间
- 检测证明超时
- **超时 = 证明失败，报编译错误并提供修复建议**
- 平衡证明能力和编译速度

## 设计决策（2026-02）

**旧设计（已废弃）**：证明超时 → 自动插入运行时检查

**新设计**：证明失败 → 编译错误 + 修改建议

理由：
- 符合"坚如磐石"设计哲学：所有安全问题必须在编译期解决
- 避免 UB 风险：运行时检查无法覆盖所有场景
- 显式控制：程序员必须显式处理安全问题
...
static int verify_with_timeout(TypeChecker *checker, 
    ...
    if (proof_check_timeout(checker)) {
        // 超时 = 证明失败，报编译错误
        checker_report_proof_failure(checker, node, operation, suggestion);
        return 0;
    }
...
- [uya.md](../docs/uya.md) §14.5 - 证明场景分类
- [changelog.md](../docs/changelog.md) - 0.48 版本变更
