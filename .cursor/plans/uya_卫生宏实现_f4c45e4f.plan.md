...
使宏体内**引入的标识符**（变量、循环变量、函数名、参数）在展开时被重命名为唯一名，从而：
...
- `AST_VAR_DECL` 的 `var_decl_name`（排除会进入 `local_bindings` 的 `const x = @mc_eval` / `@mc_type`）
- `AST_FOR_STMT` 的 `for_stmt_var_name`
- `AST_FN_DECL` 的 `fn_decl_name` 及 `fn_decl_params[i].var_decl_name`

**不**重命名：宏参数名（由 `find_param_binding` 替换）、会加入 `local_bindings` 的 const 名（由 merged_ctx 替换）。

**首版范围**：仅处理 var/for/fn（及 fn 参数），不处理 `struct_decl_name`、`enum_decl_name` 等类型声明名；若后续需要可再扩展 binding 种类与 binding_sites。
...
- **binding_sites**：`(node: &ASTNode, binding_id: i32)`，覆盖 var_decl / for_stmt / fn_decl / fn 的 param 节点
...
2. **实现“收集会进入 local_bindings 的名字”**
  - 与 [src/checker/macro_expand.uya](src/checker/macro_expand.uya) 中 `extract_macro_output_with_params` 开头的逻辑一致：遍历 body 顶层，对 `const x = @mc_eval(...)` 和 `const x = @mc_type(...)` 收集 `x`，得到集合 L（可用固定大小数组或 arena 上的名字表）。
3. **实现 `build_hygiene_data(body, param_names, local_binding_names, arena, expansion_id)`**
  - 遍历宏体 AST，收集所有绑定点（var_decl / for_stmt / fn_decl / fn_decl_params[i]）；若绑定名不在 `param_names` 且不在 `local_binding_names`，则分配下一个 `binding_id`，写入 `binding_sites`，并在 arena 上生成 `new_name = name + "__hyg_" + expansion_id + "_" + binding_id`，写入 `rename_map[binding_id]`。
  - 再遍历宏体做简单作用域解析：维护“当前作用域内名字 -> binding_id”的栈或链式表；进入块/函数体时压栈，退出时弹栈；遇到标识符使用时，解析到 binding_id（或标记为 param / free），写入 `use_sites`。
...
4. **两处展开入口均调用 `build_hygiene_data` 并写入 ctx**
  - 在 [src/checker/macro_expand.uya](src/checker/macro_expand.uya) 的 `expand_macros_in_node_simple` 中，**两处**构建 `ctx` 并调用 `extract_macro_output_with_params` 的路径都要在调用 extract 之前执行 hygiene 预处理：
    - **AST_CALL_EXPR 路径**（约 2755–2778 行）：识别到宏调用、构建好 `ctx.bindings` 之后，先得到 `local_binding_names`，再调用 `build_hygiene_data(...)`，将结果填入 `ctx` 的 hygiene 字段。
    - **AST_METHOD_BLOCK 路径**（约 2862–2912 行）：对 struct 返回宏展开时，在构建好 `ctx.bindings` 之后同样先得到 `local_binding_names`，调用 `build_hygiene_data(...)` 并填入 `ctx`，否则方法块内展开的宏会缺少卫生。
  - `local_binding_names` 可与 `extract_macro_output_with_params` 内逻辑抽成共用函数（如 `collect_local_binding_names(body, arena)`）。
...
1. **在 `deep_copy_ast_with_params` 中应用卫生**
  ...
  - **AST_FN_DECL**：对 `fn_decl_name` 查 `binding_sites` 并可能替换；对 `fn_decl_params[i]` 的拷贝在递归到子节点时，每个 param 节点（var_decl）同样会走 AST_VAR_DECL 分支，因此只要 binding_sites 包含这些 param 节点即可。
  - 递归时继续传入同一 `ctx`，保证子 AST 中标识符和绑定点使用同一套表。
2. **在 `deep_copy_ast_with_field_subst` 中同步卫生逻辑**
  - 该函数用于 `for info.fields` 的展开，也会拷贝宏体片段。为其传入的 `ctx` 已包含 hygiene 数据（来自同一次展开的 `merged_ctx`），在拷贝 `AST_IDENTIFIER`、`AST_VAR_DECL`、`AST_FOR_STMT`、`AST_FN_DECL` 时应用与 `deep_copy_ast_with_params` 相同的查找与重命名规则，保证 for 展开后的块内标识符也卫生。
...
2. **规范与文档**
  - 在 [docs/uya.md](docs/uya.md) 宏系统章节（§25）中增加“卫生宏”小节：说明宏体内引入的变量/循环变量/函数名/参数会做卫生重命名，避免与调用处名字捕获；宏参数与 `const x = @mc_eval/@mc_type` 的语义不变。
...
| 收集 local_binding 名    | 同上                                                                                           | 可抽成 `collect_local_binding_names(body, arena)`，供 build_hygiene 与 extract 共用                 |
...
| 拷贝时查表重命名              | 同上，`deep_copy_ast_with_params` 约 1109–1340 行                                                 | 仅当 ctx 含 hygiene 数据时做查找；对 AST_IDENTIFIER / VAR_DECL / FOR_STMT / FN_DECL 分支增加 hygiene 查找与赋值 |
...
- **作用域实现**：需正确处理块、for、fn 体、以及 fn 参数的作用域顺序（参数先于函数体）。进入 `AST_FN_DECL` 时先遍历 `fn_decl_params` 把参数名加入当前作用域，再遍历 `fn_decl_body`。首版可仅对 BLOCK 与 FN_DECL 压栈作用域，与语法规范（如 [docs/grammar_formal.md](docs/grammar_formal.md)）中作用域规则一致。
- **性能**：use_sites / binding_sites 线性查找；若后续宏体变大，可改为按 (line, column) 或节点指针的简单哈希。
- **唯一名格式**：`__hyg_%d_%d` 需避免与用户合法标识符冲突；若规范允许，可选用保留前缀（如 `__hyg`）并文档说明。错误信息中可能显示生成名（如 `tmp__hyg_7_0`），友好还原为宏内原名可留作后续优化。
...
- 新增卫生宏测试：宏内局部变量与调用处同名不互相干扰；多次展开同一宏得到不同唯一名。
